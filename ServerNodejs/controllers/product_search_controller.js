...
router.get('/sub/:id/:condition', async function (req, res) {
  let cd="";
  const condition=req.params.condition;
  var active_5=true;
  var active_up_4= true;
  ...
  const total=await categoryModel.countByCatConditionSub(catId,cd);
  let nPages=Math.floor(total/paginate.limit);
  if (total %paginate.limit>0)
  {
    nPages++;
  ...
  const list = await categoryModel.getStarCourseConditionSub(catId,offset,cd);
  if (req.session.authUser!=null){
    const listBuy=await categoryModel.getBuyList(req.session.authUser.user_id);
    await categoryModel.checkBill(list,listBuy);
    await categoryModel.checkisHaving(req.session.cart,list);
...
router.get('/sub/:id/mode/:condition', async function (req, res) {
  const condition=req.params.condition;
  let cd=""
  let mode="";
  let ms=false;
  ...
  const list = await categoryModel.mostByCatIDConditionSub(catId, offset,cd,mode);
  if (req.session.authUser!=null){
    const listBuy=await categoryModel.getBuyList(req.session.authUser.user_id);
    await categoryModel.checkBill(list,listBuy);
    await categoryModel.checkisHaving(req.session.cart,list);
